...
$query = "SELECT COUNT(*) AS total FROM admin_baiviet WHERE id_benh = ' $id_benh' ";
  ...
  public function getDSTinTuc() {
    $query = "SELECT id, title, slug, id_benh, id_khoa, tieu_de, img FROM admin_baiviet WHERE id_benh = '42' ORDER BY admin_baiviet.id DESC";
    $result = $this->db->select($query);
    if($result) {
        $data = [];
        // Sử dụng vòng lặp để lấy tất cả các dòng dữ liệu
        while($row = $result->fetch_assoc()) {
            $data[] = $row;
        }
        return $data; // Trả về mảng các dòng dữ liệu
    } else {
